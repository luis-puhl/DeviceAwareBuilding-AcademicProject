...
possuía regulador de tensão *onboard*, utilizava-se o próprio conectado a uma
...
módulo, é necessário o uso de uma placa *breadboard*, como a imagem a seguir.
...
A programação foi feita de primeiro modo através de um *firmware* genérico
chamado AT. Este é um conjunto de instruções enviados via serial para o módulo
ESP que permite configurá-lo. A IDE Arduino e o Cool Term possuem um emulador de
terminal serial que aceita os comandos AT e os envia direto para a serial.Além
disso, utilizou-se a linguagem C que foi compilada na IDE Arduino e enviada ao
ESP.
...
essencial para a descoberta de pacotes que trafegam entre dispositivos e a os
...
de *toolchains* (conjunto de ferramentas para desenvolvimento de software) da
empresa Espressif e de um usuário do Github, muito utilizado para projetos de
ESPs, Paulo Sokolovsky (*pfalcon*). Ambas as *toolchains* são SDKs de código
aberto. Os *scripts* foram feitos na linguagem C, compilados nessas SDKs e
transferidos para os módulos ESP. O maior problema dessas SDKs foi a
...
de fazer VMs, mas a máquina também não possui virtualização.
...
por $ 35,00 (Raspberry Foundation) e, no Brasil, por R$ 250,00 (Mercado Livre).
...
de um carregador de *smartphone* que não forneceu os amperes necessários.
